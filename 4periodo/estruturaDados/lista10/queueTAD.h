 ...
 * ----------------------------------------------------------
 * Recebe uma "queue", um "elemento" e uma "prioridade", e enfileira o elemento na
 * posição adequada da fila de acordo com sua prioridade. Elementos com prioridades
 * menores são posicionados à frente de elementos com prioridades maiores. Para
 * elementos com a mesma prioridade, a ordem de inserção é mantida (FIFO).
 * Os possíveis retornos são:
 *
 *     a) QUEUE_OK: operação realizada com sucesso;
 *     b) QUEUE_ERRO_QUEUE: queue inválida;
 *     c) QUEUE_ERRO_ARGUMENTO: elemento inválido;
 *     d) QUEUE_ERRO_ALOCACAO: erro na alocação de memória.
 */
queue_status
priority_enqueue(queueTAD queue, const elementoT elemento, const int prioridade);
