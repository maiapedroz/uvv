...
{
    return priority_enqueue(queue, elemento, INT_MAX);
}

// Nova função priority_enqueue
queue_status
priority_enqueue(queueTAD queue, const elementoT elemento, const int prioridade)
{
    ...
    // Se o novo elemento tem maior prioridade que o primeiro
    if (prioridade < queue->inicio->prioridade)
    {
    ...
    // Se foi inserido no final, atualiza o ponteiro fim
